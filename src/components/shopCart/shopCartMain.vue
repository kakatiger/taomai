    ...
    <div class="cartMain">
      <ul>
          ...
          <li class="pay">
              ...
              <el-button type="primary" @click='goToOrder()'>下单</el-button>
          </li>
      </ul>
    </div>
</template>
<script>
    export default {
        ...
        methods:{
            ...
            handleChange(cid,index){//点击数量加减时同步更新到服务器
            ...
            goToOrder(){//点击下单跳转...
                ...
                this.$router.push('order?json='+str)
                }else{
                    this.$message.error({
                             message: '请选择要购买的商品'
                        }); 
        ...
        computed:{
                ...
                cartCount(){//购物车数量，同步到seesionStorage
                    var count=0
                    for(var i=0;i<this.list.length;i++){
        ...
        mounted(){      
                ...
                this.axios.get('cart/select',{params:{uid}}).then((res)=>{//加载完成后即显示对应用户的购物车
                   this.list=res.data
                   var sum=0
                   for(var i=0;i<this.list.length;i++){
